...
function calcOverallScore(...rawScores) {
  const scores = rawScores
    .map((s) => (s === undefined || s === null || s === "" ? null : Number(s)))
    .filter((n) => Number.isFinite(n));
  if (scores.length === 0) return 0;
  for (const n of scores) {
    if (n < 1 || n > 5) {
      const err = new Error("Each score must be a number between 1 and 5");
      err.status = 400;
      throw err;
    }
  }
...
exports.create = async (req, res) => {
    try {
        ...
        let overall_score;
        try {
            overall_score = calcOverallScore(
                teamwork_score, communication_score, productivity_score,
                punctuality_score, leadership_score
            );
        } catch (e) {
...
exports.update = async (req, res) => {
    try {
        ...
        const tw = teamwork_score ?? existing[0].teamwork_score;
        const cm = communication_score ?? existing[0].communication_score;
        const pr = productivity_score ?? existing[0].productivity_score;
        const pu = punctuality_score ?? existing[0].punctuality_score;
        const ld = leadership_score ?? existing[0].leadership_score;
        let overall_score;
        try {
            overall_score = calcOverallScore(tw, cm, pr, pu, ld);
        } catch (e) {
            if (e.status === 400) return res.status(400).json({ message: e.message });
            throw e;
        }
